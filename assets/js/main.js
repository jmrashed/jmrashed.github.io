...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupMobileMenuToggle() {
    ...
    mobileMenuButton.addEventListener('click', () => {
      ...
      console.log(
        `Mobile menu toggled. Current state visible: ${!mobileMenu.classList.contains('hidden')}`
      );
    });
  ...
  function animateCounter(element, target, duration = 2000) {
    ...
    console.log(
      `Animating counter for element: ${element.id || element.className}, Target: ${target}`
    );
    updateCounter();
  }
  ...
  function setupRippleEffect() {
    ...
    document.querySelectorAll('button, .btn').forEach(button => {
      button.addEventListener('click', function (e) {
        ...
        console.log(
          `Ripple effect triggered on button: ${this.textContent.trim() || this.className}`
        );
      });
    });
    ...
    $(function () {
      ...
      if ($dots.length && $dots.children().length !== total) {
        ...
        for (let i = 0; i < total; i++) {
          const btn = $(
            `<button class="w-3 h-3 rounded-full ${i === 0 ? 'bg-blue-500' : 'bg-gray-600'}" data-index="${i}"></button>`
          );
          $dots.append(btn);
        }
      }
      ...
      entries => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            ...
            console.log(
              `Element with data-animate="${animation}" intersected. Animation triggered.`
            );
          }
        });
      },
...
<a href="${project.CodeLink || '#'}" target="_blank"
  ...
  <i class="fab fa-github"></i> Code
      ...
      $projectsContainer.html(
        '<p class="text-red-400">Failed to load projects. Please try again later.</p>'
      );
    });
  }
  ...
  function loadAchievements() {
    $.getJSON('assets/data/achivements.json', function (data) {
      ...
      $('#achievements-container').html(
        `<p class="text-red-400">Failed to load achievements data.</p>`
      );
    });
  }
  ...
  function loadExperience() {
    $.getJSON('assets/data/experience.json', function (data) {
      ...
      if (data && data.length > 0) {
        $.each(data, function (index, exp) {
          ...
          const stackHTML =
            exp.stack && exp.stack.length > 0
              ? `<div class="mt-2"><h4 class="uppercase font-semibold text-blue-400 mb-2">Stack</h4>${exp.stack
                  .map(
                    tech =>
                      `<span class="inline-block bg-purple-600/20 text-purple-300 px-2 py-1 mr-1 rounded-full text-xs">${tech}</span>`
                  )
                  .join('')}</div>`
              : '';
          ...
          const skillsHTML =
            exp.skills && exp.skills.length > 0
              ? `<div class="mt-2"> <h4 class="uppercase font-semibold text-blue-400 mb-2"> Skills</h4> ${exp.skills
                  .map(
                    skill =>
                      `<span class="inline-block bg-green-600/20 text-green-300 px-2 py-1 mr-1 rounded-full text-xs">${skill}</span>`
                  )
                  .join('')}</div>`
              : '';
        ...
        $container
          .find('.collapsibleResponsibility .collapsible-toggle')
          .off('click')
          .on('click', function () {
            const $btn = $(this);
      ...
      $('#appendByExperienceContent').html(
        `<p class="text-red-400">Failed to load experience data.</p>`
      );
    });
  }
  ...
  function loadMenu() {
    $.getJSON('assets/data/menu.json', function (menuData) {
      ...
      if (desktopMenuContainer.length === 0 || mobileMenuContainer.length === 0) {
        console.warn(
          'Menu containers not found. Ensure #desktop-menu-container and #mobile-menu-container exist.'
        );
        return;
      }
  ...
  function loadBlogsMainPage() {
    ...
    $.getJSON('assets/data/blogs.json', function (blogs) {
      ...
      $blogsGrid.html(
        '<p class="text-red-500 text-center col-span-full">Error loading blogs. Please try again later.</p>'
      );
    });
  }
  ...
  function loadBlogDetails() {
    ...
    if (!blogId) {
      blogDetailsContainer.html(
        '<p class="text-red-500 text-center">Blog ID not found in URL.</p>'
      );
    ...
    $.getJSON('assets/data/blogs.json', function (data) {
      ...
      blogDetailsContainer.html(
        '<p class="text-red-500 text-center">Error loading blog details. Please try again later.</p>'
      );
    });
  }
  ...
  function loadProjectDetails() {
    ...
    if (isNaN(projectId)) {
      projectDetailsContainer.html(
        '<p class="text-red-500 text-center">Project ID not found in URL.</p>'
      );
    ...
    $.getJSON('assets/data/projects.json', function (data) {
      ...
      if (project) {
        ...
        $('#competitors').text(
          (project['Key Competitors'] && project['Key Competitors'].join(', ')) || 'N/A'
        );
        ...
        $('#deployment-options').text(
          (project['Deployment Options'] && project['Deployment Options'].join(', ')) || 'N/A'
        );
        $('#scalability').text(project.Scalability || 'N/A');
        $('#security-features').text(
          (project['Security Features'] && project['Security Features'].join(', ')) || 'N/A'
        );
        $('#integration-capabilities').text(
          (project['Integration Capabilities'] && project['Integration Capabilities'].join(', ')) ||
            'N/A'
        );
        $('#support-languages').text(
          (project['Support Languages'] && project['Support Languages'].join(', ')) || 'N/A'
        );
        ...
        $.getJSON('assets/data/case-studies.json', function (caseStudiesData) {
          ...
          if (relatedCaseStudies.length > 0) {
            ...
            console.log(
              `Loaded ${relatedCaseStudies.length} related case studies for project ID ${projectId}.`
            );
          } else {
          ...
          $('#no-related-case-studies')
            .removeClass('hidden')
            .text('Error loading related case studies.');
        });
      } else {
      ...
      projectDetailsContainer.html(
        '<p class="text-red-500 text-center">Error loading project details. Please try again later.</p>'
      );
    });
  }
  ...
  <h3 class="text-2xl font-bold">
    ${caseStudy.title || 'Untitled Case Study'}
      ...
      $caseStudiesContainer.html(
        '<p class="text-red-400">Failed to load case studies. Please try again later.</p>'
      );
    });
  }
  ...
  function loadCaseStudyDetails() {
    ...
    if (isNaN(caseStudyId)) {
      caseStudyDetailsContainer.html(
        '<p class="text-red-500 text-center">Case Study ID not found in URL.</p>'
      );
    ...
    $.getJSON('assets/data/case-studies.json', function (data) {
      ...
      if (caseStudy) {
        ...
        $('#case-study-client-industry').text(
        ...
        if (caseStudy.technologiesUsed && caseStudy.technologiesUsed.length > 0) {
          caseStudy.technologiesUsed.forEach(tech => {
            technologiesUsedList.append(
              `<span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">${tech}</span>`
            );
          });
        }
        ...
        caseStudyDetailsContainer.html(
          '<p class="text-red-500 text-center">Case study not found.</p>'
        );
      ...
      caseStudyDetailsContainer.html(
        '<p class="text-red-500 text-center">Error loading case study details. Please try again later.</p>'
      );
    });
  }
  ...
  function initAllAfterNav() {
    ...
    setupScrollAnimations('.skill-card, .project-card', cardInitialStyles, null, {
      threshold: 0.1,
    });
    ...
    try {
      ...
      console.error('Failed to initialize testimonials slider:', e);
    }
    setupPreloader();
...
$(document).ready(function () {
  ...
  $.getJSON('assets/data/socialLinks.json', function (socialLinks) {
    ...
    let tooltipEl = null;
    $('.social-links')
      .on('mouseenter focus', '.social-icon', function (e) {
